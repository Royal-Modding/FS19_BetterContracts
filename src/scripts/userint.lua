...
function BetterContracts:getDimensions(field, verbose)
	...
	for i = 1, numd do
		...
		if xmin < dimensions.minX then dimensions.minX = xmin; end;
		if xmax > dimensions.maxX then dimensions.maxX = xmax; end;
		if zmin < dimensions.minZ then dimensions.minZ = zmin; end;
		if zmax > dimensions.maxZ then dimensions.maxZ = zmax; end;
		...
		if verbose then
			print(string.format("(%.1f, %.1f) (%.1f, %.1f) (%.1f, %.1f) width: %.2f, height: %.2f, area: %.2f",
				x0,z0,x1,z1,x2,z2, widthLength, heightLength, math.abs(area)))
		end
	end
...
function BetterContracts:getFromVehicle(cat, m)
	...
	if cat == 4 then
		...
		for _, vec in ipairs(vehicles) do
			if ListUtil.hasListElement({"MOWERS","MOWERVEHICLES"}, vec.categoryName) and 
				tonumber(vec.specs.workingWidth) > wwidth then
		...
		for _, vec in ipairs(vehicles) do
			...
			if cat == 1 and self:isHarvester(vtype) or cat == 2 and self:isSpreader(vtype)
				or cat == 3 and self:isSimple(vtype) then
				speed = vec.specs.speedLimit
	...
	if wwidth == nil then
		...
		if self.debug and cat ~= 2 then
			print(string.format(
			"--warning BetterContracts:getFromVehicle() could not find appropriate vehicle in mission %s. Last vehicle: %s %s",
			m.id, vtype, spr))
		end
	end
...
function BetterContracts:spreadMission(m, wid, hei, vWorkwidth, vSpeed)
	...
	for j = 1, maxj do
		...
		if j==3 then 										-- values from mission vehicle
			if vtype == "fertilizerspreaders" then 
				usage[j], price[j] = self.sprUse[1], self.prices[1]-- fertilizer 
				ftex3 = fer 
			else 
				usage[j], price[j] = self.sprUse[2], self.prices[2]-- liquid fertilizer
				ftex3 = liq
			end
		end
		if mtyp == self.mtype.SOW then
	...
	local jbest = 1
	for j = 1, maxj do
		if cost[j] > cost[jbest] then jbest = j end
	end
	if jbest == 2 then ftext = liq
	elseif jbest == 3 then ftext = ftex3
	end
	return {
		miss 	= m,
		ftype 	= ftext,
